...
var 
    ...
    sf        (long_name='Ajuste Estoque-Fluxo')
    ...
    cre_y     (long_name='Crescimento do PIB')
;
...
varexo 
    ...
    e_b       (long_name='Choque na dinâmica da dívida bruta do governo')
;

% Parâmetros

parameters beta alpha delta theta rho_A rho_cg rho_xg A_ss 
           tauc_ss tauw_ss tauk_ss tauo_ss rho_c rho_w rho_k rho_to tauob rho_sf 
           psi alpha1 deltag xg_ss kg_ss c_ss n_ss k_ss x_ss y_ss b_ss sf_ss 
...
tauw_ss   = 0.15;
tauk_ss   = 0.15;
R_y       = 0.3934; %0.3750; % Receitas fiscais/PIB
cg_y      = 0.3830; %0.3859; %0.3830; % Consumo Público/PIB
xg_y      = 0.0040; %0.0040; % Investimento Público/PIB 
b_y       = 0.9219; %0.8440; % Dívida/PIB
x_y       = 0.1642; % Investimento Privado/PIB
cre_y_ss  = 0.0236; % Crescimento do PIB0.02822224
...
[name='Ajuste Estoque-Fluxo']
sf = rho_sf * sf(-1) + (1 - rho_sf) * sf_ss;
...
%--------------------------------------------------------------------------
% 8. Parâmetros a Estimar
%--------------------------------------------------------------------------

estimated_params;
stderr e_A,    inv_gamma_pdf, 0.01, inf;
stderr e_cg,   inv_gamma_pdf, 0.01, inf;
stderr e_xg,   inv_gamma_pdf, 0.01, inf; 
stderr e_tauc, inv_gamma_pdf, 0.01, inf;  
stderr e_tauw, inv_gamma_pdf, 0.01, inf;  
stderr e_tauk, inv_gamma_pdf, 0.01, inf;
stderr e_b,    inv_gamma_pdf, 0.01, inf;
tauob,         beta_pdf,      0.031, 0.01;  
rho_A,         beta_pdf,      0.69, 0.1; 
rho_cg,        beta_pdf,      0.47, 0.1;
rho_xg,        beta_pdf,      0.47, 0.1; 
rho_c,         beta_pdf,      0.41, 0.1; 
rho_w,         beta_pdf,      0.41, 0.1;  
rho_k,         beta_pdf,      0.41, 0.1;
end;

%--------------------------------------------------------------------------
% 9. Variáveis Observáveis
%--------------------------------------------------------------------------

varobs cre_y R_gdp G_gdp b_gdp;

%--------------------------------------------------------------------------
% 10. Estimação Bayesiana
%--------------------------------------------------------------------------

estimation(order = 1, datafile = 'data.csv');
